  ...
  {#if !isInitialized}
    <div in:fly={{ duration: 600 }} out:slide={{ duration: 400 }}>
      {#if globalError}
        ...
        <div class="min-h-screen bg-gradient-to-br from-rose-50 to-red-50 flex items-center justify-center p-6">
          <div class="w-full max-w-md bg-white rounded-2xl shadow-2xl p-8 space-y-6">
            <!-- Icono de error -->
            <div class="flex justify-center">
              <div class="w-16 h-16 bg-gradient-to-r from-rose-500 to-red-500 rounded-full flex items-center justify-center shadow-lg">
            ...
            <div class="text-center space-y-2">
              <h2 class="text-xl font-bold text-slate-900">Oops, algo salió mal</h2>
              <p class="text-slate-600 text-sm">{globalError?.message || 'Error inesperado en la aplicación'}</p>
            </div>
      ...
      {:else}
        ...
        <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center p-6">
          <div class="w-full max-w-md space-y-6">
            <!-- Logo skeleton -->
            <div class="flex justify-center">
              <div class="w-20 h-20 bg-gradient-to-r from-indigo-300 to-purple-300 rounded-2xl animate-pulse shadow-lg"></div>
            </div>
            
            <!-- Content skeleton -->
            <div class="space-y-4">
              <div class="h-4 bg-gradient-to-r from-slate-200 to-slate-300 rounded-full animate-pulse"></div>
              <div class="h-4 bg-gradient-to-r from-slate-200 to-slate-300 rounded-full w-3/4 animate-pulse"></div>
              <div class="h-4 bg-gradient-to-r from-slate-200 to-slate-300 rounded-full w-1/2 animate-pulse"></div>
            </div>
            ...
            <div class="relative overflow-hidden h-2 bg-slate-200 rounded-full">
              <div class="absolute left-0 top-0 h-full bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full animate-pulse" style="width: 70%;"></div>
            </div>
          </div>
        </div>
      {/if}
